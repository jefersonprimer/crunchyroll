...
export default function SeasonClient({ params, messages }: { params: { season: string }, messages: any }) {
  ...
  const seasonAnimes = data.animes.filter((anime: any) => {
    ...
    return anime.seasons.some((seasonData: any) => {
      return (
        seasonData.season?.toUpperCase() === seasonName.toUpperCase() &&
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-4 md:mb-0">{t("simulcasts.season.titlePage")}</h1>
          {renderDropdown()}
        </div>
  ...
  return (
    <div>
      <div className="container mx-auto px-4 py-16 w-[80%]">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <h1 className="text-[26px] font-weight-700 text-[#FFFFFF] mb-4 md:mb-0">{t("simulcasts.season.titlePage")}</h1>
          {renderDropdown()}
        </div>
