...
import { DropdownIcon, SearchIcon, UserIcon, WatchlistIcon } from "@/app/components/layout/components/icons/HeaderIcons";
import type { UserProfile } from "@/types/header";
import SideBar from "./SideBar";


export default function Header() {
  // Pega o locale da rota (ajuste conforme o padrão do seu projeto)
  const router = useRouter();
  // Exemplo: /en/games, /pt-br/games, etc.
  const params = useParams();
  const pathname = usePathname();
  const locale = params.locale as string;

  // Estados simulados para evitar erros de linter
  ...
  const [isAccountModalOpen] = useState(false);
  const [isAnonymousModalOpen] = useState(false);
  const onUserClick = () => { /* abrir modal de conta futuramente */ };
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  return (
    <>
      <header className="bg-[#23252B] shadow-md z-60">
        <div className="flex items-center justify-between">
          {/* Left Menu */}
          <div>
            <button
              className={`flex items-center cursor-pointer gap-2 justify-center w-[120px] h-[60px] transition-colors duration-200 
                ${isSidebarOpen ? 'bg-[#141519]' : 'hover:bg-[#141519] active:bg-[#141519]'}
              `}
              onClick={() => setIsSidebarOpen(true)}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="w-6 h-6" fill="currentColor">
                <path d="M21 18a1 1 0 0 1 0 2H3a1 1 0 0 1 0-2zm0-7a1 1 0 0 1 0 2H3a1 1 0 0 1 0-2zm0-7a1 1 0 0 1 0 2H3a1 1 0 0 1 0-2z" fillRule="evenodd"></path>
              </svg>
          ...
          <div className="items-center no-underline w-[172px] h-[60px] hidden lg:flex">
            <Link href="/" className="flex items-center no-underline w-[172px] h-[60px] px-4.5">
              <svg 
