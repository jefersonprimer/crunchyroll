...
import GenrePopularClient, { type AudioFilter } from './GenrePopularClient';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; genre: string }> }) {
  ...
  return {
    title: t('genrePopularPage.title', { genre: genreInfo.pt }),
    description: t('genrePopularPage.title', { genre: genreInfo.pt }),
  };
}

export default async function GenrePopularPage({ params, searchParams }: { params: Promise<{ locale: string; genre: string }>, searchParams: Promise<{ lang?: string }> }) {
  ...
  const audioFilter = (lang || 'subtitled_dubbed') as AudioFilter;

  return (
    <div>
      <Header />
      <GenrePopularClient animes={animes} genre={genre} genreInfo={genreInfo} error={error} audioFilter={audioFilter} />
      <Footer />
    </div>
  );
}
