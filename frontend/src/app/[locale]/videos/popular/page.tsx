...
export default function PopularPage() {
  ...
  const { activeDropdown, toggleDropdown } = useDropdown();
   ...
   const { data, loading, error } = useQuery(GET_POPULAR_ANIMES);

  useEffect(() => {
    if (data?.popularAnimes) {
      let filtered = data.popularAnimes;
  ...
  return (
    <div>
      <ClientMetadata
        title="Animes e Filmes Mais Populares - Crunchyroll"
        description="Assista a Animes e Filmes Mais Populares - Crunchyroll"
      />
    <Header />
