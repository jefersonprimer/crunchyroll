    ...
    <div className="bg-[#23252B] p-[20px] w-[720px] h-[336px]">
      ...
      <div className='flex flex-col justify-center items-center'>
        ...
        <div className="mb-[60px]">
          <label className="block text-[#A0A0A0]">{t('listName')}</label>
          <input
            type="text"
            value={newListName}
            onChange={(e) => setNewListName(e.target.value)}
            className="w-[560px] p-[10px] bg-[#23252B] text-white border-b border-b-[#59595B] outline-none focus:border-b-[#FF640A] focus:outline-none"
          />
        </div>
        <div className="flex justify-end gap-[10px]">
        <button onClick={() => onSave(newListName)} className="py-[8px] px-[32px] cursor-pointer bg-[#FF640A] text-[#000000] font-bold">{t('renameButton')}</button>
        <button onClick={onClose} className="py-[8px] px-[32px] cursor-pointer border-1 border-[#FF640A] text-[#FF640A] font-bold">{t('cancel')}</button>
        </div>
      </div>
    </div>
  </div>
